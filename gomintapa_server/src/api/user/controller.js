...
exports.register = async (req, res) => {
    ...
    if (count > 0) {
        return res.send({ result: "fail", message: '중복된 이메일이존재합니다.' });
    }
    ...
    if (affectedRows > 0) {
        const data = await generateToken({ id: insertId, name });
...
exports.login = async (req, res) => {
    ...
    if(item == null) {
        ...
        const data = await generateToken({id:item.id, name: item.name});
        res.send({result:'ok', access_token: data})
    }
